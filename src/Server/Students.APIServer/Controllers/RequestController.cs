...
public class RequestController : GenericAPiController<Request>
{
  ...
  /// <summary>
  /// Поиск заявок (возвращает DTO, как и пагинация).
  /// </summary>
  [HttpGet("SearchDTO")]
  public async Task<IActionResult> SearchDTO([FromQuery] string searchWithoutType)
  {
    try
    {
      ...
      var items = await this._requestRepository.SearchRequestsDTO(search);
      return this.Ok(items);
    }
    catch (Exception e)
    {
