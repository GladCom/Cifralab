...
function Row(props) {
  ...
  const handleChandeRequests = (id) => {
    ...
    if (row?.requests == null || row?.requests == undefined) row.requests = [];
    if (row?.requests.indexOf(request) == -1)
      setRow(row?.requests.push(request));
    else setRow(row?.requests.splice(row?.requests.indexOf(request), 1));
  };

  const handleChandeGroups = (id) => {
    let group = groups.filter((x) => x.id == id[1])[0];
    if (row?.groups == null || row?.groups == undefined) row.groups = [];
    if (row?.groups.indexOf(group) == -1) setRow(row?.groups.push(group));
    else setRow(row?.groups.splice(row?.groups.indexOf(group), 1));
  };
  ...
  return (
    <React.Fragment>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        ...
        <TableCell align="center" sx={{ m: 1, width: 150 }} title={row?.snils}>
          <Input
            value={row?.snils}
            readOnly={edit}
            onChange={(e) => setRow((row.snils = e.target.value))}
            sx={{ m: 1, width: "10rem" }}
          />
        </TableCell>
        <TableCell
        ...
        >
          <Input
            ...
            readOnly={edit}
            onChange={(e) => setRow((row.documentNumber = e.target.value))}
            sx={{ m: 1, width: "5rem" }}
          />
        </TableCell>
        <TableCell
          align="center"
          sx={{ m: 1, width: "10rem" }}
          title={row?.documentSeries}
        >
          <Input
            value={row?.documentSeries}
            readOnly={edit}
            onChange={(e) => setRow((row.documentSeries = e.target.value))}
            sx={{ m: 1, width: "10rem" }}
          />
        </TableCell>
        <TableCell
        ...
        >
          <Input
            ...
            readOnly={edit}
            onChange={(e) => setRow((row.nationality = e.target.value))}
            sx={{ m: 1, width: "10rem" }}
          />
        </TableCell>
        <TableCell align="center" sx={{ m: 1, width: "10rem" }}>
          <div>
            <FormControl sx={{ m: 1, width: "10rem" }}>
              <Select
                ...
                renderValue={() =>
                  row?.requests?.map((x) => x.fullName)?.join(", ")
                }
                MenuProps={MenuProps}
                sx={{ height: 36 }}
                readOnly={edit}
                    ...
                    <TableRow key={requestsRow?.id}>
                      <TableCell component="th" scope="row" align="center">
                        <Input
                          ...
                          value={requestsRow?.educationProgramId}
                          onChange={(e) =>
                            setRow(
                              (requestsRow.educationProgramId = e.target.value)
                            )
                          }
                        />
                      </TableCell>
                      <TableCell align="center">
                        <Input
                          readOnly={editRequest}
                          value={requestsRow?.educationProgram?.name}
                          onChange={(e) =>
                            setRow(
                              (requestsRow.educationProgram.name =
                                e.target.value)
                            )
                          }
                        />
                      ...
                      <TableCell align="center">
                        <Input
                          readOnly={editRequest}
                          value={requestsRow?.interview}
                          onChange={(e) =>
                            setRow((requestsRow.interview = e.target.value))
                          }
                        />
                      </TableCell>
                    </TableRow>
                  ...
                  {row?.groups?.map((groupsRow) => (
                    <TableRow key={groupsRow?.id}>
                      <TableCell component="th" scope="row">
                        <Input
                          readOnly={editRequest}
                          value={groupsRow?.name}
                          onChange={(e) =>
                            setRow((groupsRow.name = e.target.value))
                          }
                        />
                      </TableCell>
                      <TableCell align="center">
                        <Input
                          readOnly={editRequest}
                          value={groupsRow?.educationProgram?.name}
                          onChange={(e) =>
                            setRow(
                              (groupsRow.educationProgram.name = e.target.value)
                            )
                          }
                        />
                      </TableCell>
                      <TableCell align="center">
                        <Input
                          readOnly={editRequest}
                          value={groupsRow?.startDate}
                          onChange={(e) =>
                            setRow((groupsRow.startDate = e.target.value))
                          }
                        />
                      </TableCell>
                      <TableCell align="center">
                        <Input
                          readOnly={editRequest}
                          value={groupsRow?.endDate}
                          onChange={(e) =>
                            setRow((groupsRow.endDate = e.target.value))
                          }
                        />
                      </TableCell>
                    </TableRow>
        ...
        <Table aria-label="collapsible table">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center" sx={{ width: "10rem" }}>СНИЛС</TableCell>
              <TableCell align="center" sx={{ width: "5rem" }}>Номер паспорта</TableCell>
              <TableCell align="center" sx={{ width: "10rem" }}>Серия паспорта</TableCell>
              <TableCell align="center" sx={{ width: "10rem" }}>Национальность</TableCell>
              <TableCell align="center" sx={{ width: "10rem" }}>Программа</TableCell>
              <TableCell align="center" sx={{ width: "10rem" }}>Группы</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {rows?.map((row) => (
